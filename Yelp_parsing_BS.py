...
data_s = soup.find_all("span",{"class": "main-attributes"})

data_r=soup.find_all("span",{"class": "review-count rating-qualifier"})
data_r=data_r[2:12]

names=[]

data_rat=soup.find_all("div",{"class": "biz-rating biz-rating-large clearfix"})
data_rat=data_rat[2:12]

data_pr=soup.find_all("span",{"class": "business-attribute price-range"})
data_pr=data_pr[2:12]



x=re.findall(r'<span class="indexed-biz-name">.*\n.*</span>',yelp_html)
for s in x:
    #name=re.findall(r'<span>(?P<RestName>.*)</span>',s)
    name=re.search(r'<span>(?P<RestName>.*)</span>',s)
    n= name.group('RestName')
    names.append(n)
        
for s in data_r:
    #n= name.group('RestName')
    name=re.search(r'<span class.*>\n\s*?(?P<Reviews>\d*)\s*?reviews\s*\n*?</span>',str(s))
    n= name.group('Reviews')

#x=re.findall(r'<span class="indexed-biz-name">.*\n.*</span>',yelp_html)
#name=re.findall(r'<span>(?P<RestName>.*)</span>',str(x))
print (names)
    
reviews=re.findall(r'<span class.*>\n\s*?(?P<Reviews>\d*)\s*?reviews\s*\n*?</span>',str(data_r))
reviews_n=[int(r) for r in reviews]
#print (reviews_n)

ratings=re.findall(r'<div class.*.>\n<.*\s*?title=.(?P<Rating>...).*>',str(data_rat))
#print (ratings)

pr=[]
for s in data_pr:
    s=str(s)
    pr.append(s[s.index('$'):s.index('</span>')])

#price_range=re.findall(r'<span class.*>(..)</span>',str(data_pr))
#print (price_range)
...
#dict_y={'name':'Rajat'}
#print (type(dict_y))
#assert (type(dict_y) is dict),'Not a dict'
#print ('*'*10)
for i in range(len(names)):
    dict_y={}
    dict_y['numrevs']=reviews_n[i]
    dict_y['name']=names[i]
    dict_y['stars']=ratings[i]
    dict_y['price']=pr[i]
    #dict_z=dict([('numrevs',reviews_n[i]),('name',names[i])])
    #print (type(dict_y))
    rankings.append(dict_y)

#print ('&'*100)
#for r in rankings:
#    print (type(r))
    
#if type(rankings[0]) is dict:
#    print ('Yes')
#else:
#    print ('No!!')
...
#xyz = {'numrevs': 549, 'name': "Gus’s World Famous Fried Chicken", 'stars': '4.0', 'price': '$$'}
#print (xyz)

#print (type(xyz))

#assert type(xyz) is dict, "NO! dictionaries"    
