...
Jupyter Notebook part1 (autosaved) Python 3.6 (beta)
Python 3.6 (beta) 
File
Edit
View
...
*** Contents (first 1000000 characters) ***
<!DOCTYPE html>
...
import re
re_p=re.compile('''
                <span class="indexed-biz-name">[0-9A-Za-z<>=":/.]+
                <span>\w+
                </span>
                '''
                ,re.VERBOSE)
...
a=re_p2.match(yelp_html)
x=re.search(r'<span class="indexed-biz-name">[0-9A-Za-z<>=":/.\s-]+',yelp_html)
x=re.search('<span>[\w\'\s\w]+',yelp_html)
#x=re.search('<span>(?P<RestName>.*)</span>',yelp_html)
x=re.search(r'<span class="indexed-biz-name">.*\n.*</span>',yelp_html)
x=re.findall(r'<span class="indexed-biz-name">.*\n.*</span>',yelp_html)
#x=re.findall(r'indexed-biz-name(.*\n.*\n*.*[ \t\n\r\f\v])$',yelp_html)
#x=re.findall(r'indexed-biz-name">.*\n*.*\n*.*\n*.*\n\n\n.*',yelp_html)
x1=re.findall(r'rating-large".*',yelp_html)
x1=re.findall(r'<div class="i-stars i-stars".*',yelp_html)


#x=re.findall(r'indexed-biz-name">(.*\n*.*\n*.*\n*.*\n\n\n.*)>',yelp_html)

#x=re.findall(r'indexed-biz-name>(.+?)<span class="category-str-list">',yelp_html)
...
for s in x1:
    #name=re.findall(r'title="(?P<Star>.*)</span>',s)
    #n= name.group('RestName')
    name=re.search(r'title="(?P<Star>.*)>',s)
    n= name.group('Star')
    print n
...
#name=re.search('<span>(?P<RestName>.*)</span>',x)

#print name.group('RestName')

#print yelp_html.partition('<span>')
